...
export namespace Maybe {
	...
	export const from = <T>(thing: T | Maybe<NonNullable<T>>): Maybe<NonNullable<T>> => {
		if (isMaybe(thing)) {
			return thing;
	...
	export const isSomething = <T extends {}>(thing: Maybe<T>): thing is Something<T> =>
...
class Something<T> {
	...
	andThen<U>(executor: (value: T) => U | Maybe<NonNullable<U>>): Maybe<NonNullable<U>> {
