...
function map_insert(key2, value, map5) {
  return map5.set(key2, value);
}
...
// build/dev/javascript/box/box.mjs
var Inputs = class extends CustomType {
  constructor(href) {
    super();
    this.href = href;
  }
...
function d_button(attrs, children2) {
  return button(prepend(class$(" btn "), attrs), children2);
}
function view(model) {
  let count = to_string2(model.count);
  return div(
    toList([class$(" container p-4 mx-auto ")]),
    toList([
      d_button(toList([on_click(new Incr())]), toList([text(" + ")])),
      p(toList([]), toList([text(count)])),
      d_button(toList([on_click(new Decr())]), toList([text(" - ")]))
    ])
  );
}
function main() {
  ...
  let inputs = new Inputs(href);
  let $ = start2(app, "#app", inputs);
  if (!$.isOk()) {
    throw makeError(
      "assignment_no_match",
      "box",
